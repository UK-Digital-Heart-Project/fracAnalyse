...
def dicm_dict(vendor='SIEMENS', flds=None):
    # Initialize the dictionary
    dict = {'vendor': vendor}

    # Common DICOM dictionary fields
    C = [
        ...
        ['0018', '9087', 'FD', 'B_value'],
        ['0018', '9089', 'FD', 'DiffusionGradientDirection'],
    ...
    if vendor.upper().startswith('SIEMENS'):
        ...
        C.extend([
            ...
            ['0019', '10BB', 'DS', 'DiffusionDirectionX'],
            ['0019', '10BC', 'DS', 'DiffusionDirectionY'],
            ['0019', '10BD', 'DS', 'DiffusionDirectionZ'],
            ...
            ['0019', '10E0', 'DS', 'NumberOfDiffusionDirection'],
            ['0019', '10E2', 'DS', 'VelocityEncodeScale'],
            ['0019', '10F2', 'SS', 'FastPhases'],
            ['0019', '10F9', 'DS', 'TransmitGain'],
            ['0020', '9301', 'FD', 'ImagePositionPatient'],
            ['0020', '9302', 'FD', 'ImageOrientationPatient'],
    ...
    elif vendor.upper().startswith('PHILIPS'):
    C.extend([
        ...
        ['2001', '1020', 'LO', 'ScanningSequence'],
        ['2001', '1021', 'CS', 'SPIR'],
        ['2001', '1022', 'FL', 'WaterFatShift'],
        ['2001', '1023', 'DS', 'MRSeriesFlipAngle'],
        ...
        ['2001', '1082', 'IS', 'MRSeriesEchoTrainLength'],
        ['2001', '1083', 'DS', 'MRSeriesImagingFrequency'],
        ['2001', '1084', 'DS', 'MRSeriesInversionTime'],
        ['2001', '1085', 'DS', 'MRSeriesMagneticFieldStrength'],
        ['2001', '1086', 'IS', 'MRSeriesNrOfPhaseEncodingSteps'],
        ['2001', '1087', 'SH', 'MRSeriesNucleus'],
        ['2001', '1088', 'DS', 'MRSeriesNumberOfAverages'],
        ['2001', '1089', 'DS', 'MRSeriesPercentPhaseFieldOfView'],
        ['2001', '108A', 'DS', 'MRSeriesPercentSampling'],
        ['2001', '108B', 'SH', 'MRSeriesTransmittingCoil'],
        ...
        ['2005', '1030', 'FL', 'MRSeriesRepetitionTime'],
        ...
        ['2005', '106F', 'CS', 'MRAcquisitionType'],
    ...
    group = np.array([int(x[0], 16) for x in C], dtype=np.uint16)
    element = np.array([int(x[1], 16) for x in C], dtype=np.uint16)
    tag = group.astype(np.uint32) * 65536 + element.astype(np.uint32)
    vr = [x[2] for x in C]
    name = [x[3] for x in C]
    ...
    if flds is not None and len(flds) > 0:
        flds = list(flds)
        ind = np.array([name_i in flds for name_i in name])

        dict['fields'] = flds
        dict['tag'] = tag[ind]
        dict['vr'] = np.array(vr)[ind].tolist()
        dict['name'] = np.array(name)[ind].tolist()
        dict['group'] = group[ind]
        dict['element'] = element[ind]

    # Sorting and unique tags
    unique_tags, unique_indices = np.unique(tag, return_index=True)
    dict['tag'] = unique_tags
    dict['vr'] = np.array(vr)[unique_indices].tolist()
    dict['name'] = np.array(name)[unique_indices].tolist()
    dict['group'] = group[unique_indices]
    dict['element'] = element[unique_indices]
